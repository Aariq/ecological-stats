...
```
  ...
  c. Fit a negative binomial GLM with latitude as a predictor and survey effort as an offset.  Report the AIC and theta, the dispersion parameter.
  
```{r}
library(MASS)
m3 <- glm.nb(count ~ Latitude + offset(log(hours)), data = woody)
AIC(m3)
summary(m3)
```
...
```
  ...
  e. What ecological process might result in the winning model?
...
```{r}
m0 <- glm.nb(count ~ 1 + offset(log(hours)), data = woody)
lmtest::lrtest(m0, m3)
```
