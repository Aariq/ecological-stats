...
```{r}
#plot as proportion rather than count with stat="count" and y = after_stat(prop)
#after_stat() access variables calculated by the stat specified in the geom.  I.e. stat = "count" produces both a count variable and a prop variable behind the scenes before plotting.
tibble(x = rpois(10000, exp(coef(m1)))) %>% #generate random numbers
  ggplot() +
  geom_bar(stat = "count", aes(x, y = after_stat(prop), group = 1),
