...
ggplot(df, aes(x = x, y = y)) +
  geom_col() +
  ylim(0,1) +
  labs(x = "X", y = "P(n)", title = "p(success) = 0.6")
```
...
ggplot(df, aes(x = p, y = loglik)) +
  ...
  labs(x = "value of p", y = "log-likelihood")
```
...
Seedlings in year 0 is our `x` ($k$), or number of events.  Our size, $N$, is how many seeds we think we started out with, depending on the lag (of 1, 2, 3, or 4 years). 
